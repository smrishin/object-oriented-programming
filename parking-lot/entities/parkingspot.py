...
class ParkingSpot:
    def __init__(self, spot_id: str, spot_size: VehicleSize) -> None:
    ...
    def can_fit_vehicle(self, vehicle: Vehicle):
        if self.is_occupied:
            return False
        curr_vehicle_size = vehicle.get_size()
        if curr_vehicle_size == VehicleSize.SMALL:
            return self.spot_size == VehicleSize.SMALL
        elif curr_vehicle_size == VehicleSize.MEDIUM:
            return self.spot_size in [VehicleSize.MEDIUM, VehicleSize.LARGE]
        elif curr_vehicle_size == VehicleSize.LARGE:
